...
class search_space():
    ...
    def gradient_descent_path(self, eta=1, obj_alpha=0.05, lam=10, mu=10, alpha_step=0.01, max_iter=1000, sec = 60): # <- 1st order
    ...
    def newton_method(self, lam, mu, alpha=0.1, max_iter = 1000, sec = 60): # <- 2nd order
        t0 = time.perf_counter()
        ...
        while not stop:
            ...
            new_h = V @ np.diag(np.maximum(np.abs(eig), 1e-8)) @ V.T
            delta = np.linalg.solve(new_h, g)
...
def basic_GD(start, goal, test=1, steps= 100, max_iter=1000, sec = 60):
    ...
    search = initialize(start, goal, test, steps)
    search.gradient_descent_path(eta = 1, obj_alpha=obj_alpha, lam=lam, mu=mu, alpha_step=1e-2, max_iter=max_iter, sec = sec)
    return search
...
def GD_adam(start, goal, test=1, steps= 100, max_iter=1000, sec = 60):
    ...
    search = initialize(start, goal, test, steps)
    search.adam(obj_alpha=obj_alpha, alpha_step=1e-3, beta=1e-3,  lam=lam, mu=mu, max_iter=max_iter, sec = sec)
    return search
