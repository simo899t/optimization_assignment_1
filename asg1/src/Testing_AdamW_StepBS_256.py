...
optimizers = []
optimizers.append(optim.AdamW(models[0].parameters(), lr=0.001, weight_decay = 0.1))
optimizers.append(optim.AdamW(models[1].parameters(), lr=0.001, weight_decay = 0.1))
optimizers.append(optim.AdamW(models[2].parameters(), lr=0.001, weight_decay = 0.1))


modelSTD = LeNet5().to(device)
optimizerSTD = optim.AdamW(modelSTD.parameters(), lr=0.001, weight_decay= 0.1)
...
for j, model in enumerate(models):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=False)
    batch_scheduler = StepBS(train_loader, step_size=step_sizes[j], gamma=2, max_batch_size=60000)
...
# print(f"Train_losses size: {train_losses[j,k].shape}")
# print(f"Train_losses input size: {torch.tensor(train_loss_epoch, dtype=torch.float32).shape}")
test_losses[3] = torch.tensor(test_loss_epoch, dtype=torch.float32)
train_losses[3] = torch.tensor(train_loss_epoch, dtype=torch.float32)
accuracy[3] = torch.tensor(accuracy_epoch, dtype=torch.float32)
...
# Plot Train Loss
ax[0].plot(train_losses[0, :, 0], train_losses[0, :, 1], label="Train Loss - StepBS (step=2)", color="blue")
ax[0].plot(train_losses[1, :, 0], train_losses[1, :, 1], label="Train Loss - StepBS (step=5)", color="red")
ax[0].plot(train_losses[2, :, 0], train_losses[2, :, 1], label="Train Loss - StepBS (step=10)", color="green")
ax[0].plot(train_losses[3, :, 0], train_losses[3, :, 1], label="Train Loss - No StepBS", color="lime")
ax[0].set_ylabel("Loss")
ax[0].legend(loc="upper right")
ax[0].grid(True)

# Plot Test Accuracy
ax[1].plot(accuracy[0, :, 0], accuracy[0, :, 1], label="Accuracy - StepBS (step=2)", color="blue")
ax[1].plot(accuracy[1, :, 0], accuracy[1, :, 1], label="Accuracy - StepBS (step=5)", color="red")
ax[1].plot(accuracy[2, :, 0], accuracy[2, :, 1], label="Accuracy - StepBS (step=10)", color="green")
ax[1].plot(accuracy[3, :, 0], accuracy[3, :, 1], label="Accuracy - No StepBS", color="lime")
ax[1].set_ylabel("Test Accuracy")
ax[1].legend(loc="lower right")
ax[1].grid(True)

# Plot Test Loss
ax[2].plot(test_losses[0, :, 0], test_losses[0, :, 1], label="Test Loss - StepBS (step=2)", color="blue")
ax[2].plot(test_losses[1, :, 0], test_losses[1, :, 1], label="Test Loss - StepBS (step=5)", color="red")
ax[2].plot(test_losses[2, :, 0], test_losses[2, :, 1], label="Test Loss - StepBS (step=10)", color="green")
ax[2].plot(test_losses[3, :, 0], test_losses[3, :, 1], label="Test Loss - No StepBS", color="lime")
...
plt.tight_layout()
plt.savefig('Comparing_StepBS_AdamW_withStartBS=256.png')
plt.show()
